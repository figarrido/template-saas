...
<!--
  TEMPLATE NOTE — this file is the design-system source of truth for the repo,
  following the getdesign.md / DESIGN.md standard (YAML tokens + prose). The
  template ships Notion as the *neutral reference*, the same way it ships Stripe
  as the reference billing adapter. A derived project REPLACES this file with
  its own DESIGN.md and re-derives the tokens in packages/ui/src/globals.css +
  packages/config/tailwind/preset.ts. See docs/architecture/07-frontend.md.

  The template wires only the CORE token layer (neutrals, primary, radius, type
  scale, semantics, font → Inter). The decorative brand layer below (navy hero
  band, pastel card-tint-* colors, brand color spectrum, product-specific button
  variants) is documented for derived projects to opt into, and is NOT shipped.

  Independent analysis of publicly observable patterns, for inspiration only.
  Not affiliated with or endorsed by Notion; Notion and its logo are trademarks
