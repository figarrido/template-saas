...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  const issues = plan.output.issues as {body: string; comments: string[]; labels: string[]; number: number; title: string; id: string; branch: string}[];
  ...
  await sandcastle.run({
    ...
    // CAVEAT: on a macOS host the merger therefore can't run the suite with
    // correct native binaries. If you need test-verified merges, run this
    // orchestrator from a Linux host (where the host node_modules matches the
    // sandbox), or isolate this phase in a worktree like Phase 2.
    sandbox: docker(),
    name: "merger",
    maxIterations: 1,
    agent: sandcastle.claudeCode("claude-sonnet-4-6"),
